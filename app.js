...
function showPanel(panelId) { document.querySelectorAll(".panel").forEach(p => p.classList.add("hidden")); document.getElementById(panelId).classList.remove("hidden"); document.querySelectorAll(".nav-item").forEach(b => b.classList.toggle("active", b.dataset.panel === panelId)); window.scrollTo({ top: 0, behavior: "smooth" }); }
...
function renderCycleDetails(wk) { const info = cycleInfo[wk]; els.cycleDetails.innerHTML = `<h3>${info.title}</h3><p class="muted">${info.text}</p><p class="small muted">Rule: hit all target reps → increase. Miss once → repeat. Miss twice → reduce 10% and rebuild.</p>`; }
...
async function loadHistory() { const [workouts, runs] = await Promise.all([supabaseClient.from("workout_logs").select("*").eq("user_id", currentUser.id).order("created_at", { ascending: false }).limit(10), supabaseClient.from("run_logs").select("*").eq("user_id", currentUser.id).order("created_at", { ascending: false }).limit(5)]); if (workouts.error || runs.error) { els.historyList.innerHTML = `<p class="muted">${workouts.error?.message || runs.error?.message}</p>`; return; } const workoutHtml = (workouts.data || []).map(row => `<article class="history-item"><strong>${row.date} · ${row.workout_type}</strong><span class="muted small">Week ${row.cycle_week} ${row.is_deload ? "· Deload" : "· Build"}</span><p class="muted">${row.notes || ""}</p></article>`).join(""); const runHtml = (runs.data || []).map(row => `<article class="history-item"><strong>${row.date} · ${row.run_type}</strong><span class="muted small">${row.completed_minutes || 0}/${row.target_minutes || 0} min · RPE ${row.rpe || "—"}</span><p class="muted">${row.notes || ""}</p></article>`).join(""); els.historyList.innerHTML = workoutHtml + runHtml || `<p class="muted">No history yet.</p>`; }
